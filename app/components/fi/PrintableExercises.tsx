  ...
  const handlePrint = () => {
    ...
    if (printWindow) {
      const title = exerciseType === 'single' 
        ? `Harjoitukset Taulu ${selectedTable}`
        : exerciseType === 'mixed'
        ? 'Sekoitetut Kertotaulut'
      ...
      printWindow.document.write(`
        ...
        <html>
          ...
          <body>
            <button onclick="window.print()" class="print-button no-print">🖨️ Tulosta</button>
            <h1>${title}</h1>
            <div class="info">
              <p>Päivämäärä: ${new Date().toLocaleDateString('fi-FI')}</p>
              <p>Tehtävien Määrä: ${questionCount}</p>
            </div>
            <div class="questions">
              ${questions.map((q, i) => `
                <div class="question">
                  <div class="question-number">Tehtävä ${i + 1}</div>
      ...
      <div className="max-w-6xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 gap-8 mb-8">
            ...
            <div className="space-y-6">
              <div>
                ...
                <div className="space-y-2">
                  <label className="flex items-center p-3 border-2 border-gray-200 rounded-lg cursor-pointer hover:border-blue-500 transition-colors">
                    ...
                    />
                    <div>
                      <div className="font-medium text-slate-900">Yksi Taulu</div>
                      <div className="text-sm text-slate-600">Yksi tietty kertotaulu</div>
                    </div>
                  </label>
                  ...
                  <label className="flex items-center p-3 border-2 border-gray-200 rounded-lg cursor-pointer hover:border-blue-500 transition-colors">
                    ...
                    />
                    <div>
                      <div className="font-medium text-slate-900">Väli</div>
                      <div className="text-sm text-slate-600">Taulut tietyllä välillä</div>
                    </div>
                  </label>
                  ...
                  <label className="flex items-center p-3 border-2 border-gray-200 rounded-lg cursor-pointer hover:border-blue-500 transition-colors">
                    ...
                    />
                    <div>
                      <div className="font-medium text-slate-900">Sekoitettu</div>
                      <div className="text-sm text-slate-600">Satunnaisesti kaikista tauluista</div>
                    </div>
                  </label>
                </div>
              </div>
              ...
              <div>
                <label className="flex items-center p-4 bg-amber-50 border-2 border-amber-200 rounded-lg cursor-pointer">
                  ...
                  />
                  <div>
                    <div className="font-medium text-slate-900">Sisällytä Vastausavain</div>
                    <div className="text-sm text-slate-600">Lisää vastaukset sivun loppuun</div>
                  </div>
                </label>
              </div>
            </div>
            ...
            <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl p-6 border-2 border-blue-200">
              <h3 className="text-lg font-bold text-slate-900 mb-4 flex items-center gap-2">
                <span>👁️</span> Esikatselu
              </h3>
              
              <div className="bg-white rounded-lg p-4 mb-4 space-y-3">
                <div className="flex justify-between items-center">
                  <span className="text-sm text-slate-600">Tyyppi:</span>
                  <span className="font-semibold text-slate-900">
                    {exerciseType === 'single' 
                      ? `Taulu ${selectedTable}`
                      : exerciseType === 'range'
                      ? `Taulut ${rangeStart}-${rangeEnd}`
                      : 'Sekoitettu'}
                  </span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-sm text-slate-600">Tehtävien Määrä:</span>
                  <span className="font-semibold text-slate-900">{questionCount}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-sm text-slate-600">Vastausavain:</span>
                  <span className={`font-semibold ${includeAnswers ? 'text-green-600' : 'text-slate-400'}`}>
                    {includeAnswers ? 'Kyllä ✓' : 'Ei ✗'}
                  </span>
                </div>
              </div>

              <div className="bg-white rounded-lg p-4 space-y-2">
                <div className="text-sm font-semibold text-slate-700 mb-2">Esimerkkejä Tehtävistä:</div>
                {[...Array(3)].map((_, i) => {
              ...
              <div className="mt-4 p-3 bg-blue-100 rounded-lg border border-blue-300">
                <p className="text-sm text-blue-900">
                  💡 <strong>Vinkki:</strong> Voit käyttää selaimen tulostustoimintoa tallentaaksesi PDF-muodossa.
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="mt-8 grid md:grid-cols-3 gap-6">
          <div className="bg-white rounded-xl p-6 border-2 border-gray-200">
            ...
            <h4 className="font-bold text-slate-900 mb-2">Perustaulut (1-10)</h4>
          ...
          <div className="bg-white rounded-xl p-6 border-2 border-gray-200">
            <div className="text-3xl mb-3">📱</div>
            <h4 className="font-bold text-slate-900 mb-2">Kaikki taulut (1-100)</h4>
            <p className="text-sm text-slate-600">
              Kattava kokoelma
            </p>
          </div>
          <div className="bg-white rounded-xl p-6 border-2 border-gray-200">
            <div className="text-3xl mb-3">💾</div>
            <h4 className="font-bold text-slate-900 mb-2">Tyhjät mallit</h4>
            <p className="text-sm text-slate-600">
              Harjoitteluun
            </p>
          </div>
