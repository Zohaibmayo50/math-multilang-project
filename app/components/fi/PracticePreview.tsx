...
export default function PracticePreview({ rangeStart = 1, rangeEnd = 10 }: PracticePreviewProps) {
  ...
  const exercises = [
    {
      level: 'Helppo',
      range: rangeStart === rangeEnd ? `Taulu ${rangeStart}` : `Taulut ${rangeStart}-${Math.min(rangeStart + 4, rangeEnd)}`,
      questions: '20 Kysymystä',
      color: 'bg-green-100 text-green-700 border-green-300'
    },
    {
      level: 'Keskitaso',
      range: rangeStart === rangeEnd ? `Taulu ${rangeStart}` : `Taulut ${rangeStart}-${Math.min(rangeStart + 7, rangeEnd)}`,
      questions: '30 Kysymystä',
      color: 'bg-yellow-100 text-yellow-700 border-yellow-300'
    },
    {
      level: 'Vaikea',
      range: rangeStart === rangeEnd ? `Taulu ${rangeStart}` : `Taulut ${Math.max(rangeStart, rangeEnd - 5)}-${rangeEnd}`,
      questions: '40 Kysymystä',
      color: 'bg-orange-100 text-orange-700 border-orange-300'
    },
    {
      level: 'Asiantuntija',
      range: rangeStart === rangeEnd ? `Taulu ${rangeStart}` : `Taulut ${rangeStart}-${rangeEnd}`,
      questions: '50 Kysymystä',
    ...
    <section id="practice" className="section-container bg-gradient-to-br from-slate-50 to-blue-50 border-t border-gray-200">
      <div className="max-w-6xl mx-auto">
        <h2 className="text-slate-900 mb-4 text-center">
          Harjoittele Kertotauluja {rangeStart}–{rangeEnd}
        </h2>
        
        <p className="text-center text-slate-700 max-w-3xl mx-auto mb-12 text-lg">
          Testaa taitojasi välittömällä palautteella
        </p>
            ...
            <div className="bg-white rounded-2xl shadow-xl p-8 border border-gray-200">
              <div className="flex justify-between items-center mb-8">
                <div className="text-center">
                  <div className="text-sm text-slate-600 mb-1">Oikein</div>
                  <div className="text-2xl font-bold text-green-600">{score.correct}</div>
                </div>
                <div className="text-center">
                  <div className="text-sm text-slate-600 mb-1">Yhteensä</div>
                  <div className="text-2xl font-bold text-blue-600">{score.total}</div>
                </div>
                <div className="text-center">
                  <div className="text-sm text-slate-600 mb-1">Onnistuminen</div>
                    ...
                    {feedback === 'correct' ? (
                      ...
                      <div>
                        ...
                        <div className="text-base text-slate-600">
                          Oikea vastaus: {question.num1 * question.num2}
                        </div>
                      </div>
                    )}
                  </div>
          ...
          <div className="max-w-3xl mx-auto">
            <div className="bg-white rounded-2xl shadow-xl p-8 border border-gray-200">
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-2xl font-bold text-blue-600">📝 Harjoitukset</h3>
              ...
              <div className="mb-6">
                <div className="flex justify-between items-center mb-2">
                  <span className="text-sm font-medium text-slate-600">
                    Kysymys {currentQuestionIndex + 1} / {exerciseQuestions.length}
                  </span>
                  <span className="text-sm font-medium text-slate-600">
                    Pisteet: {exerciseQuestions.filter(q => q.correct === true).length}
                  </span>
                </div>
              ...
              <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl p-12 mb-6">
                ...
                <div className="flex gap-4 items-center justify-center">
                  ...
                  >
                    {currentQuestionIndex < exerciseQuestions.length - 1 ? 'Seuraava' : 'Valmis'}
                  </button>
                </div>
              </div>
            </div>
          ...
          <div className="max-w-3xl mx-auto">
            <div className="bg-white rounded-2xl shadow-xl p-8 border border-gray-200">
              <div className="text-center py-8">
                ...
                <h3 className="text-3xl font-bold text-slate-900 mb-4">Harjoitus Valmis!</h3>
                ...
                <div className="grid grid-cols-3 gap-4 max-w-xl mx-auto mb-8">
                  ...
                  <div className="bg-red-50 rounded-lg p-4">
                    <div className="text-sm text-slate-600 mb-1">Väärin</div>
                    <div className="text-3xl font-bold text-red-600">
                      {exerciseQuestions.filter(q => q.correct === false).length}
                    </div>
                  </div>
                  <div className="bg-blue-50 rounded-lg p-4">
                    <div className="text-sm text-slate-600 mb-1">Menestys</div>
                ...
                {exerciseQuestions.filter(q => q.correct === false).length > 0 && (
                  <div className="mt-8 text-left max-w-xl mx-auto">
                    <h4 className="font-semibold text-slate-900 mb-4">Väärät Vastaukset:</h4>
                        ...
                        q.correct === false && (
                          <div key={idx} className="bg-red-50 border border-red-200 rounded-lg p-3 flex justify-between items-center">
                            ...
                            <span className="text-red-600 font-medium">
                              Sinun vastauksesi: {q.userAnswer}
                            </span>
                          </div>
                        )
                      )}
          ...
          <div className="flex items-start">
            ...
            <div>
              ...
              <p className="text-slate-700">
                Harjoittelu ei tarkoita vain nopeutta. Mieti jokaista kysymystä huolellisesti, 
                analysoi väärät vastauksesi ja tunnista, mitkä kertotaulut 
                tuottavat sinulle vaikeuksia. Säännöllinen 10-15 minuutin 
                päivittäinen harjoittelu on tehokkain menetelmä pitkäaikaiseen oppimiseen.
              </p>
            </div>
          </div>
        </div>
