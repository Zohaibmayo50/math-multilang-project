...
export default function NumberGamesEn({ number }: NumberGamesProps) {
      ...
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-8">
          <h2 className="text-3xl md:text-4xl font-bold text-slate-900 mb-4">
            🎮 Times Table {number} Games
          </h2>
          <p className="text-lg text-slate-700">
            Practice the {number} times table with fun animated games!
          </p>
        </div>
        ...
        <div className="flex flex-wrap gap-2 sm:gap-3 justify-center mb-8">
          ...
          >
            🎈 <span className="hidden sm:inline">Pop Balloons</span><span className="sm:hidden">Balloons</span>
          </button>
          <button
          ...
          >
            🏎️ <span className="hidden sm:inline">Racing Game</span><span className="sm:hidden">Race</span>
          </button>
          <button
          ...
          >
            🧠 <span className="hidden sm:inline">Memory Game</span><span className="sm:hidden">Memory</span>
          </button>
        </div>
          ...
          <div className="bg-gradient-to-b from-sky-100 to-sky-300 rounded-2xl p-4 sm:p-8 shadow-xl min-h-[400px] sm:min-h-[600px] relative overflow-hidden">
            <div className="absolute top-2 sm:top-4 left-2 sm:left-4 right-2 sm:right-4 flex justify-between items-center z-10">
              <div className="bg-white/90 backdrop-blur-sm px-2 sm:px-4 py-1 sm:py-2 rounded-lg font-bold text-sm sm:text-lg">
                Score: <span className="text-pink-600">{bpScore}</span>
              </div>
              <div className="bg-white/90 backdrop-blur-sm px-2 sm:px-4 py-1 sm:py-2 rounded-lg font-bold text-sm sm:text-lg">
                Missed: <span className={bpMissed >= 3 ? 'text-red-600' : 'text-orange-600'}>{bpMissed}/5</span>
              </div>
            </div>
            ...
            {!bpGameActive ? (
              <div className="flex flex-col items-center justify-center h-[350px] sm:h-[500px]">
                ...
                <h3 className="text-xl sm:text-2xl font-bold mb-3 sm:mb-4 px-4">Balloon Pop Game</h3>
                ...
                {bpScore > 0 && (
                  <div className="mt-4 sm:mt-6 text-base sm:text-lg">
                    Last Score: <span className="font-bold text-pink-600">{bpScore}</span>
                  </div>
                )}
              </div>
            ) : (
        ...
        {activeGame === 'race' && (
          <div className="bg-white rounded-2xl p-8 shadow-xl">
            <div className="text-center mb-6">
              <h3 className="text-2xl font-bold text-blue-600 mb-2">🏎️ Racing Game</h3>
              <p className="text-slate-600">Answer correctly to win the race!</p>
            </div>
            ...
            {!raceGameActive || raceCar.position >= 90 ? (
              <div className="text-center py-8 sm:py-12">
                ...
                {raceCar.position >= 90 ? (
                  <>
                    <h3 className="text-2xl sm:text-3xl font-bold text-green-600 mb-3 sm:mb-4 px-4">🎉 Congratulations! You won!</h3>
                    <div className="text-xl sm:text-2xl mb-4 sm:mb-6">
                      Total Score: <span className="font-bold text-blue-600">{raceScore}</span>
                    </div>
                  </>
                ) : (
                  <h3 className="text-xl sm:text-2xl font-bold mb-4 px-4">Ready for the race?</h3>
                )}
                <button
                ...
                >
                  {raceCar.position >= 90 ? 'Play Again' : 'Start Race'} 🏎️
                </button>
              </div>
            ) : (
              <div>
                ...
                <div className="text-center text-base sm:text-lg font-semibold">
                  Score: <span className="text-blue-600">{raceScore}</span>
                </div>
              </div>
            )}
          </div>
        ...
        {activeGame === 'memory' && (
          <div className="bg-white rounded-2xl p-8 shadow-xl">
            <div className="text-center mb-6">
              <h3 className="text-2xl font-bold text-purple-600 mb-2">🧠 Memory Game</h3>
              <p className="text-slate-600">Find the matching pairs!</p>
            </div>

            {cards.length === 0 ? (
              <div className="text-center py-8 sm:py-12">
                <div className="text-6xl sm:text-8xl mb-4 sm:mb-6">🃏</div>
                <h3 className="text-xl sm:text-2xl font-bold mb-3 sm:mb-4 px-4">Match the Cards</h3>
                <p className="text-sm sm:text-base text-slate-600 mb-4 sm:mb-6 px-4">Match multiplications with their results!</p>
                <button
              ...
              <div>
                <div className="flex justify-center gap-4 sm:gap-6 mb-4 sm:mb-6">
                  <div className="text-base sm:text-lg font-semibold">
                    Matched: <span className="text-green-600">{memoryScore}/6</span>
                  </div>
                  <div className="text-base sm:text-lg font-semibold">
                    Moves: <span className="text-purple-600">{memoryMoves}</span>
                  </div>
                </div>
                ...
                {memoryScore === 6 && (
                  <div className="text-center mt-6 sm:mt-8">
                    <h3 className="text-2xl sm:text-3xl font-bold text-green-600 mb-3 sm:mb-4">🎉 Congratulations!</h3>
                    <p className="text-lg sm:text-xl mb-3 sm:mb-4">
                      You completed it in {memoryMoves} moves!
                    </p>
                    <button
                      onClick={initMemoryGame}
