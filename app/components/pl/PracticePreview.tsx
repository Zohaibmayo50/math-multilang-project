...
export default function PracticePreviewPl({ rangeStart = 1, rangeEnd = 10 }: PracticePreviewProps) {
  ...
  const exercises = [
    {
      level: 'Łatwy',
      range: rangeStart === rangeEnd ? `Tabliczka ${rangeStart}` : `Tabliczki ${rangeStart}-${Math.min(rangeStart + 4, rangeEnd)}`,
      questions: '20 Pytań',
      color: 'bg-green-100 text-green-700 border-green-300'
    },
    {
      level: 'Średni',
      range: rangeStart === rangeEnd ? `Tabliczka ${rangeStart}` : `Tabliczki ${rangeStart}-${Math.min(rangeStart + 7, rangeEnd)}`,
      questions: '30 Pytań',
      color: 'bg-yellow-100 text-yellow-700 border-yellow-300'
    },
    {
      level: 'Trudny',
      range: rangeStart === rangeEnd ? `Tabliczka ${rangeStart}` : `Tabliczki ${Math.max(rangeStart, rangeEnd - 5)}-${rangeEnd}`,
      questions: '40 Pytań',
      color: 'bg-orange-100 text-orange-700 border-orange-300'
    },
    {
      level: 'Ekspert',
      range: rangeStart === rangeEnd ? `Tabliczka ${rangeStart}` : `Tabliczki ${rangeStart}-${rangeEnd}`,
      questions: '50 Pytań',
    ...
    <section id="practice" className="section-container bg-gradient-to-br from-slate-50 to-blue-50 border-t border-gray-200">
      <div className="max-w-6xl mx-auto">
        <h2 className="text-slate-900 mb-4 text-center">
          Ćwicz Tabliczki Mnożenia od {rangeStart} do {rangeEnd}
        </h2>
        
        <p className="text-center text-slate-700 max-w-3xl mx-auto mb-12 text-lg">
          Sprawdź swoje umiejętności z natychmiastową informacją zwrotną
        </p>
            ...
            <div className="bg-white rounded-2xl shadow-xl p-8 border border-gray-200">
              <div className="flex justify-between items-center mb-8">
                <div className="text-center">
                  <div className="text-sm text-slate-600 mb-1">Poprawne</div>
                  <div className="text-2xl font-bold text-green-600">{score.correct}</div>
                </div>
                <div className="text-center">
                  <div className="text-sm text-slate-600 mb-1">Razem</div>
                  <div className="text-2xl font-bold text-blue-600">{score.total}</div>
                </div>
                <div className="text-center">
                  <div className="text-sm text-slate-600 mb-1">Skuteczność</div>
                    ...
                    {feedback === 'correct' ? (
                      ...
                      <div>
                        ...
                        <div className="text-base text-slate-600">
                          Poprawna odpowiedź: {question.num1 * question.num2}
                        </div>
                      </div>
                    )}
                  </div>
          ...
          <div className="max-w-3xl mx-auto">
            <div className="bg-white rounded-2xl shadow-xl p-8 border border-gray-200">
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-2xl font-bold text-blue-600">📝 Ćwiczenia</h3>
              ...
              <div className="mb-6">
                <div className="flex justify-between items-center mb-2">
                  <span className="text-sm font-medium text-slate-600">
                    Pytanie {currentQuestionIndex + 1} / {exerciseQuestions.length}
                  </span>
                  <span className="text-sm font-medium text-slate-600">
                    Wynik: {exerciseQuestions.filter(q => q.correct === true).length}
                  </span>
                </div>
              ...
              <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl p-12 mb-6">
                ...
                <div className="flex gap-4 items-center justify-center">
                  ...
                  >
                    {currentQuestionIndex < exerciseQuestions.length - 1 ? 'Dalej' : 'Koniec'}
                  </button>
                </div>
              </div>
            </div>
          ...
          <div className="max-w-3xl mx-auto">
            <div className="bg-white rounded-2xl shadow-xl p-8 border border-gray-200">
              <div className="text-center py-8">
                ...
                <h3 className="text-3xl font-bold text-slate-900 mb-4">Ćwiczenie Ukończone!</h3>
                ...
                <div className="grid grid-cols-3 gap-4 max-w-xl mx-auto mb-8">
                  ...
                  <div className="bg-red-50 rounded-lg p-4">
                    <div className="text-sm text-slate-600 mb-1">Błędne</div>
                ...
                {exerciseQuestions.filter(q => q.correct === false).length > 0 && (
                  <div className="mt-8 text-left max-w-xl mx-auto">
                    <h4 className="font-semibold text-slate-900 mb-4">Niepoprawne Odpowiedzi:</h4>
          ...
          <div className="flex items-start">
            ...
            <div>
              ...
              <p className="text-slate-700">
                Praktyka to nie tylko szybkość. Dokładnie przemyśl każde pytanie, 
                przeanalizuj swoje błędne odpowiedzi i zidentyfikuj, które tabliczki mnożenia 
                sprawiają trudności. Regularna praktyka przez 10-15 minut 
                dziennie to najskuteczniejsza metoda dla długoterminowej nauki.
              </p>
            </div>
          </div>
        </div>
