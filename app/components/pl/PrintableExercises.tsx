...
export default function PrintableExercisesPl({ rangeStart: defaultRangeStart = 1, rangeEnd: defaultRangeEnd = 10 }: PrintableExercisesProps) {
  ...
  const handlePrint = () => {
    ...
    if (printWindow) {
      const title = exerciseType === 'single' 
        ? `Ćwiczenia z Tabliczki Mnożenia ${selectedTable}`
        : exerciseType === 'mixed'
        ? 'Mieszane Ćwiczenia z Mnożenia'
        : `Ćwiczenia z Tabliczek Mnożenia ${rangeStart}-${rangeEnd}`
      ...
      printWindow.document.write(`
        ...
        <html>
          ...
          <body>
            <button onclick="window.print()" class="print-button no-print">🖨️ Drukuj</button>
            <h1>${title}</h1>
            <div class="info">
            ...
            <div class="questions">
              ${questions.map((q, i) => `
                <div class="question">
                  <div class="question-number">Pytanie ${i + 1}</div>
    ...
    <section id="pdf-exercises" className="section-container bg-gradient-to-br from-purple-50 to-pink-50 border-t border-gray-200">
      <div className="max-w-6xl mx-auto">
        ...
        <p className="text-center text-slate-700 max-w-3xl mx-auto mb-12 text-lg">
          Twórz spersonalizowane ćwiczenia z tabliczek mnożenia dla swoich uczniów lub dzieci. 
            ...
            <div className="space-y-6">
              <div>
                ...
                <div className="space-y-2">
                  <label className="flex items-center p-3 border-2 border-gray-200 rounded-lg cursor-pointer hover:border-blue-500 transition-colors">
                    ...
                    />
                    <div>
                      <div className="font-medium text-slate-900">Pojedyncza Tabliczka</div>
                      <div className="text-sm text-slate-600">Jedna konkretna tabliczka mnożenia</div>
                    </div>
                  </label>
                  ...
                  <label className="flex items-center p-3 border-2 border-gray-200 rounded-lg cursor-pointer hover:border-blue-500 transition-colors">
                    ...
                    />
                    <div>
                      <div className="font-medium text-slate-900">Zakres</div>
                      <div className="text-sm text-slate-600">Tabliczki mnożenia w zakresie</div>
                    </div>
                  </label>
                  ...
                  <label className="flex items-center p-3 border-2 border-gray-200 rounded-lg cursor-pointer hover:border-blue-500 transition-colors">
                    ...
                    />
                    <div>
                      <div className="font-medium text-slate-900">Mieszane</div>
                      <div className="text-sm text-slate-600">Losowe ze wszystkich tabliczek</div>
                    </div>
                  </label>
                </div>
              </div>
              ...
              <div>
                <label className="flex items-center p-4 bg-amber-50 border-2 border-amber-200 rounded-lg cursor-pointer">
                  ...
                  />
                  <div>
                    <div className="font-medium text-slate-900">Dołącz Klucz Odpowiedzi</div>
                    <div className="text-sm text-slate-600">Dodaje odpowiedzi na końcu strony</div>
                  </div>
                </label>
              </div>
            </div>
            ...
            <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl p-6 border-2 border-blue-200">
              ...
              <div className="bg-white rounded-lg p-4 mb-4 space-y-3">
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-semibold text-slate-900">
                    {exerciseType === 'single' 
                      ? `Tabliczka Mnożenia ${selectedTable}`
                      : exerciseType === 'range'
                ...
                <div className="flex justify-between items-center">
                  <span className="text-sm text-slate-600">Liczba Pytań:</span>
                  <span className="font-semibold text-slate-900">{questionCount}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-sm text-slate-600">Klucz Odpowiedzi:</span>
                  <span className={`font-semibold ${includeAnswers ? 'text-green-600' : 'text-slate-400'}`}>
                    {includeAnswers ? 'Tak ✓' : 'Nie ✗'}
                  </span>
                </div>
              </div>

              <div className="bg-white rounded-lg p-4 space-y-2">
                <div className="text-sm font-semibold text-slate-700 mb-2">Przykładowe Pytania:</div>
                {[...Array(3)].map((_, i) => {
              ...
              <div className="mt-4 p-3 bg-blue-100 rounded-lg border border-blue-300">
                <p className="text-sm text-blue-900">
                  💡 <strong>Wskazówka:</strong> Możesz zapisać jako PDF używając funkcji drukowania w przeglądarce.
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="mt-8 grid md:grid-cols-3 gap-6">
          <div className="bg-white rounded-xl p-6 border-2 border-gray-200">
            ...
            <h4 className="font-bold text-slate-900 mb-2">Konfigurowalne</h4>
            <p className="text-sm text-slate-600">
              Dostosuj liczbę pytań, tabliczki mnożenia i poziom trudności do swoich potrzeb.
            </p>
          </div>
          <div className="bg-white rounded-xl p-6 border-2 border-gray-200">
            <div className="text-3xl mb-3">📱</div>
            <h4 className="font-bold text-slate-900 mb-2">Przyjazne dla urządzeń mobilnych</h4>
            <p className="text-sm text-slate-600">
              Drukuj łatwo z telefonu, tabletu lub komputera.
            </p>
          </div>
          <div className="bg-white rounded-xl p-6 border-2 border-gray-200">
            <div className="text-3xl mb-3">💾</div>
            <h4 className="font-bold text-slate-900 mb-2">Zapisz i Udostępnij</h4>
            <p className="text-sm text-slate-600">
              Zapisz jako PDF i łatwo udostępniaj swoim uczniom.
            </p>
          </div>
        </div>
      </div>
