...
export default function NumberGamesPl({ number }: NumberGamesProps) {
      ...
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-8">
          <h2 className="text-3xl md:text-4xl font-bold text-slate-900 mb-4">
            🎮 Gry z Tabliczką Mnożenia {number}
          </h2>
          <p className="text-lg text-slate-700">
            Ćwicz tabliczkę mnożenia {number} z zabawnymi animowanymi grami!
          </p>
        </div>
        ...
        <div className="flex flex-wrap gap-2 sm:gap-3 justify-center mb-8">
          ...
          >
            🎈 <span className="hidden sm:inline">Przebijanie Balonów</span><span className="sm:hidden">Balony</span>
          </button>
          <button
          ...
          >
            🏎️ <span className="hidden sm:inline">Gra Wyścigowa</span><span className="sm:hidden">Wyścig</span>
          </button>
          <button
          ...
          >
            🧠 <span className="hidden sm:inline">Gra Pamięciowa</span><span className="sm:hidden">Pamięć</span>
          </button>
        </div>
          ...
          <div className="bg-gradient-to-b from-sky-100 to-sky-300 rounded-2xl p-4 sm:p-8 shadow-xl min-h-[400px] sm:min-h-[600px] relative overflow-hidden">
            <div className="absolute top-2 sm:top-4 left-2 sm:left-4 right-2 sm:right-4 flex justify-between items-center z-10">
              <div className="bg-white/90 backdrop-blur-sm px-2 sm:px-4 py-1 sm:py-2 rounded-lg font-bold text-sm sm:text-lg">
                Wynik: <span className="text-pink-600">{bpScore}</span>
              </div>
              <div className="bg-white/90 backdrop-blur-sm px-2 sm:px-4 py-1 sm:py-2 rounded-lg font-bold text-sm sm:text-lg">
                Pominięte: <span className={bpMissed >= 3 ? 'text-red-600' : 'text-orange-600'}>{bpMissed}/5</span>
              </div>
            </div>
            ...
            {!bpGameActive ? (
              <div className="flex flex-col items-center justify-center h-[350px] sm:h-[500px]">
                ...
                <h3 className="text-xl sm:text-2xl font-bold mb-3 sm:mb-4 px-4">Gra w Przebijanie Balonów</h3>
                <p className="text-sm sm:text-base text-slate-700 mb-4 sm:mb-6 text-center max-w-md px-4">
                  Przebijaj balony z poprawną odpowiedzią! Unikaj złych balonów i nie przegap właściwych!
                </p>
                <button
                ...
                {bpScore > 0 && (
                  <div className="mt-4 sm:mt-6 text-base sm:text-lg">
                    Ostatni Wynik: <span className="font-bold text-pink-600">{bpScore}</span>
                  </div>
                )}
              </div>
            ) : (
        ...
        {activeGame === 'race' && (
          <div className="bg-white rounded-2xl p-8 shadow-xl">
            <div className="text-center mb-6">
              <h3 className="text-2xl font-bold text-blue-600 mb-2">🏎️ Gra Wyścigowa</h3>
              <p className="text-slate-600">Odpowiadaj poprawnie, aby wygrać wyścig!</p>
            </div>
            ...
            {!raceGameActive || raceCar.position >= 90 ? (
              <div className="text-center py-8 sm:py-12">
                ...
                {raceCar.position >= 90 ? (
                  <>
                    <h3 className="text-2xl sm:text-3xl font-bold text-green-600 mb-3 sm:mb-4 px-4">🎉 Gratulacje! Wygrałeś!</h3>
                    <div className="text-xl sm:text-2xl mb-4 sm:mb-6">
                      Całkowity Wynik: <span className="font-bold text-blue-600">{raceScore}</span>
                    </div>
                  </>
                ) : (
                  <h3 className="text-xl sm:text-2xl font-bold mb-4 px-4">Gotowy na wyścig?</h3>
                )}
                <button
                ...
                >
                  {raceCar.position >= 90 ? 'Zagraj Ponownie' : 'Rozpocznij Wyścig'} 🏎️
                </button>
              </div>
            ) : (
              <div>
                ...
                <div className="text-center text-base sm:text-lg font-semibold">
                  Wynik: <span className="text-blue-600">{raceScore}</span>
                </div>
              </div>
            )}
          </div>
        ...
        {activeGame === 'memory' && (
          <div className="bg-white rounded-2xl p-8 shadow-xl">
            <div className="text-center mb-6">
              <h3 className="text-2xl font-bold text-purple-600 mb-2">🧠 Gra Pamięciowa</h3>
              <p className="text-slate-600">Znajdź pasujące pary!</p>
            </div>

            {cards.length === 0 ? (
              <div className="text-center py-8 sm:py-12">
                <div className="text-6xl sm:text-8xl mb-4 sm:mb-6">🃏</div>
                <h3 className="text-xl sm:text-2xl font-bold mb-3 sm:mb-4 px-4">Dopasuj Karty</h3>
                <p className="text-sm sm:text-base text-slate-600 mb-4 sm:mb-6 px-4">Dopasuj mnożenia z ich wynikami!</p>
                <button
              ...
              <div>
                <div className="flex justify-center gap-4 sm:gap-6 mb-4 sm:mb-6">
                  <div className="text-base sm:text-lg font-semibold">
                    Dopasowane: <span className="text-green-600">{memoryScore}/6</span>
                  </div>
                  <div className="text-base sm:text-lg font-semibold">
                    Ruchy: <span className="text-purple-600">{memoryMoves}</span>
                  </div>
                </div>
                ...
                {memoryScore === 6 && (
                  <div className="text-center mt-6 sm:mt-8">
                    <h3 className="text-2xl sm:text-3xl font-bold text-green-600 mb-3 sm:mb-4">🎉 Gratulacje!</h3>
                    <p className="text-lg sm:text-xl mb-3 sm:mb-4">
                      Ukończyłeś to w {memoryMoves} ruchach!
                    </p>
                    <button
                      onClick={initMemoryGame}
