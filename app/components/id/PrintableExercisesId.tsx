...
interface PrintableExercisesIdProps {
  rangeStart?: number
  rangeEnd?: number
}

export default function PrintableExercisesId({ rangeStart: defaultRangeStart = 1, rangeEnd: defaultRangeEnd = 10 }: PrintableExercisesIdProps) {
  ...
  const handlePrint = () => {
    ...
    if (printWindow) {
      const title = exerciseType === 'single' 
        ? `Latihan Tabel Perkalian ${selectedTable}`
        : exerciseType === 'mixed'
        ? 'Latihan Tabel Perkalian Campuran'
        : `Latihan Tabel Perkalian ${rangeStart}-${rangeEnd}`
    ...
    <section id="pdf-exercises" className="section-container bg-gradient-to-br from-purple-50 to-pink-50 border-t border-gray-200">
      <div className="max-w-6xl mx-auto">
        <h2 className="text-slate-900 mb-4 text-center">
          📄 Latihan yang Dapat Dicetak
        </h2>
        
        <p className="text-center text-slate-700 max-w-3xl mx-auto mb-12 text-lg">
          Buat latihan tabel perkalian yang disesuaikan untuk siswa atau anak-anak Anda. 
            ...
            <div className="space-y-6">
              <div>
                ...
                <div className="space-y-2">
                  <label className="flex items-center p-3 border-2 border-gray-200 rounded-lg cursor-pointer hover:border-blue-500 transition-colors">
                    ...
                    />
                    <div>
                      <div className="font-medium text-slate-900">Tabel Tunggal</div>
                      <div className="text-sm text-slate-600">Tabel perkalian tertentu</div>
                    </div>
                  </label>
                  ...
                  <label className="flex items-center p-3 border-2 border-gray-200 rounded-lg cursor-pointer hover:border-blue-500 transition-colors">
                    ...
                    />
                    <div>
                      <div className="font-medium text-slate-900">Rentang</div>
                      <div className="text-sm text-slate-600">Tabel dalam rentang tertentu</div>
                    </div>
                  </label>
                  ...
                  <label className="flex items-center p-3 border-2 border-gray-200 rounded-lg cursor-pointer hover:border-blue-500 transition-colors">
                    ...
                    />
                    <div>
                      <div className="font-medium text-slate-900">Campuran</div>
                      <div className="text-sm text-slate-600">Acak dari semua tabel</div>
                    </div>
                  </label>
                </div>
              </div>
              ...
              <div>
                <label className="flex items-center p-4 bg-amber-50 border-2 border-amber-200 rounded-lg cursor-pointer">
                  ...
                  />
                  <div>
                    <div className="font-medium text-slate-900">Tambahkan Kunci Jawaban</div>
                    <div className="text-sm text-slate-600">Menambahkan jawaban di akhir halaman</div>
                  </div>
                </label>
              </div>
            </div>
            ...
            <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl p-6 border-2 border-blue-200">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-semibold text-slate-900">
                    {exerciseType === 'single' 
                      ? `Tabel ${selectedTable}`
                      : exerciseType === 'range'
                      ? `Tabel ${rangeStart}-${rangeEnd}`
                      : 'Campuran'}
                  </span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-sm text-slate-600">Jumlah Soal:</span>
                  <span className="font-semibold text-slate-900">{questionCount}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-sm text-slate-600">Kunci Jawaban:</span>
                  <span className={`font-semibold ${includeAnswers ? 'text-green-600' : 'text-slate-400'}`}>
                    {includeAnswers ? 'Ya ✓' : 'Tidak ✗'}
                  </span>
                </div>
              </div>

              <div className="bg-white rounded-lg p-4 space-y-2">
                <div className="text-sm font-semibold text-slate-700 mb-2">Contoh Soal:</div>
                {[...Array(3)].map((_, i) => {
              ...
              <div className="mt-4 p-3 bg-blue-100 rounded-lg border border-blue-300">
                <p className="text-sm text-blue-900">
                  💡 <strong>Tips:</strong> Anda dapat menyimpannya sebagai PDF menggunakan fitur cetak browser Anda.
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="mt-8 grid md:grid-cols-3 gap-6">
          <div className="bg-white rounded-xl p-6 border-2 border-gray-200">
            ...
            <h4 className="font-bold text-slate-900 mb-2">Dapat Disesuaikan</h4>
            <p className="text-sm text-slate-600">
              Atur jumlah soal, tabel, dan tingkat kesulitan sesuai keinginan Anda.
            </p>
          </div>
          <div className="bg-white rounded-xl p-6 border-2 border-gray-200">
            <div className="text-3xl mb-3">📱</div>
            <h4 className="font-bold text-slate-900 mb-2">Ramah Mobile</h4>
            <p className="text-sm text-slate-600">
              Anda dapat mencetak dengan mudah dari ponsel, tablet, atau komputer.
            </p>
          </div>
          <div className="bg-white rounded-xl p-6 border-2 border-gray-200">
            <div className="text-3xl mb-3">💾</div>
            <h4 className="font-bold text-slate-900 mb-2">Simpan & Bagikan</h4>
            <p className="text-sm text-slate-600">
              Simpan sebagai PDF dan bagikan dengan mudah ke siswa Anda.
            </p>
          </div>
        </div>
      </div>
