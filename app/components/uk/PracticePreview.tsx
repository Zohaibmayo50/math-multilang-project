...
type ExerciseType = 'easy' | 'medium' | 'hard' | 'Експерт' | null
...
export default function PracticePreview({ rangeStart = 1, rangeEnd = 10 }: PracticePreviewProps) {
  ...
  const startExercise = (type: ExerciseType) => {
    const config = {
      ...
      Експерт: { min: rangeStart, max: rangeEnd, count: 50 }
    }
  ...
  const exercises = [
    {
      level: 'Легко',
      range: rangeStart === rangeEnd ? `Таблиця ${rangeStart}` : `Таблиці ${rangeStart}-${Math.min(rangeStart + 4, rangeEnd)}`,
      questions: '20 Запитань',
      color: 'bg-green-100 text-green-700 border-green-300'
    },
    {
      level: 'Середній',
      range: rangeStart === rangeEnd ? `Таблиця ${rangeStart}` : `Таблиці ${rangeStart}-${Math.min(rangeStart + 7, rangeEnd)}`,
      questions: '30 Запитань',
      color: 'bg-yellow-100 text-yellow-700 border-yellow-300'
    },
    {
      level: 'Важко',
      range: rangeStart === rangeEnd ? `Таблиця ${rangeStart}` : `Таблиці ${Math.max(rangeStart, rangeEnd - 5)}-${rangeEnd}`,
      questions: '40 Запитань',
      color: 'bg-orange-100 text-orange-700 border-orange-300'
    },
    {
      level: 'Експерт',
      range: rangeStart === rangeEnd ? `Таблиця ${rangeStart}` : `Таблиці ${rangeStart}-${rangeEnd}`,
      questions: '50 Запитань',
    ...
    <section id="practice" className="section-container bg-gradient-to-br from-slate-50 to-blue-50 border-t border-gray-200">
      <div className="max-w-6xl mx-auto">
        <h2 className="text-slate-900 mb-4 text-center">
          Практикувати Таблицю множення від {rangeStart} до {rangeEnd}
        </h2>
        
        <p className="text-center text-slate-700 max-w-3xl mx-auto mb-12 text-lg">
          Практика дуже важлива pro upevnění naučené násobilky. 
          Ви можете практикувати zábavným způsobem за допомогою наступних інструментів.
        </p>
            ...
            <div className="bg-white rounded-2xl shadow-xl p-8 border border-gray-200">
              <div className="flex justify-between items-center mb-8">
                <div className="text-center">
                  <div className="text-sm text-slate-600 mb-1">Правильно</div>
                  <div className="text-2xl font-bold text-green-600">{score.correct}</div>
                </div>
                <div className="text-center">
                  <div className="text-sm text-slate-600 mb-1">Всього</div>
                  <div className="text-2xl font-bold text-blue-600">{score.total}</div>
                </div>
                <div className="text-center">
                  <div className="text-sm text-slate-600 mb-1">Успішність</div>
                  ...
                  <div className={`mt-6 text-center text-xl font-semibold ${
                    ...
                    {feedback === 'correct' ? (
                      <div className="flex items-center justify-center gap-2">
                        ...
                        <span>Чудово! Правильна відповідь!</span>
                      </div>
                    ) : (
                      <div>
                        ...
                        <div className="text-base text-slate-600">
                          Правильна відповідь: {question.num1 * question.num2}
                        </div>
                      </div>
                    )}
                  </div>
        ...
        {activeTab === 'exercises' && !activeExercise && (
          <div className="grid gap-6 md:grid-cols-2 max-w-4xl mx-auto">
            {exercises.map((exercise, index) => (
              <div 
                key={index}
                onClick={() => startExercise(['easy', 'medium', 'hard', 'Експерт'][index] as ExerciseType)} 
          ...
          <div className="max-w-3xl mx-auto">
            <div className="bg-white rounded-2xl shadow-xl p-8 border border-gray-200">
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-2xl font-bold text-blue-600">📝 Вправи</h3>
              ...
              <div className="mb-6">
                <div className="flex justify-between items-center mb-2">
                  <span className="text-sm font-medium text-slate-600">
                    Запитання {currentQuestionIndex + 1} / {exerciseQuestions.length}
                  </span>
                  <span className="text-sm font-medium text-slate-600">
                    Правильно: {exerciseQuestions.filter(q => q.correct === true).length}
                  </span>
                </div>
              ...
              <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl p-12 mb-6">
                ...
                <div className="flex gap-4 items-center justify-center">
                  ...
                  >
                    {currentQuestionIndex < exerciseQuestions.length - 1 ? 'Далі' : 'Завершити'}
                  </button>
                </div>
              </div>
            </div>
          ...
          <div className="max-w-3xl mx-auto">
            <div className="bg-white rounded-2xl shadow-xl p-8 border border-gray-200">
              <div className="text-center py-8">
                ...
                <h3 className="text-3xl font-bold text-slate-900 mb-4">Вправи Dokončeno!</h3>
                ...
                <div className="grid grid-cols-3 gap-4 max-w-xl mx-auto mb-8">
                  ...
                  <div className="bg-red-50 rounded-lg p-4">
                    <div className="text-sm text-slate-600 mb-1">Неправильно</div>
                ...
                {exerciseQuestions.filter(q => q.correct === false).length > 0 && (
                  <div className="mt-8 text-left max-w-xl mx-auto">
                    <h4 className="font-semibold text-slate-900 mb-4">Неправильно Odpovědi:</h4>
          ...
          <div className="flex items-start">
            ...
            <div>
              ...
              <p className="text-slate-700">
                Вправи neznamená pouze rychlost. Pečlivě přemýšlejte o každé otázce, 
                analyzujte své Неправильно odpovědi a rozpoznejte, které násobilky 
                vám dělají potíže. Pravidelné Вправи 10-15 minut denně je 
